...
_END;
        ...
        <div id='settingsTabs'>
            <ul>
                <a href='account.php'><li id='differentLI'>Account</li></a>
       ...
       <div class='settingsContentClass'>
           <form action='saveChangesAccount.php' method='POST'>
            <h1 class='headerClass'>Account</h1>
            <label for='accountUsername' class='labelClass'>Username</label><br/>
            <?php
                echo <<<_END
            <input type='text' maxlength='15' class='textInput' value={$_COOKIE['username']} id='accountUsername' name='accountUsername'/><br/>
            <span>http://9gag.com/u/{$_COOKIE['username']}</span><br/>
...
_END;
            ...
            ?>
            <label for='accountEmail' class='labelClass'>Email</label><br/>
            <?php
                echo <<<_END
            <input type='email' class='textInput' value='{$_COOKIE['email']}'/><br/>
_END;
            ?>
            
            <span>Email will not be displayed publicly</span><br/><br/>
            <label for='showNSFW' class='labelClass'>Show NSFW Posts</label><br/>
            <select name='showNSFW'>
                <option value='Off' id='Off'>Off</option>
                <option value='On' id='On'>On</option>
            </select>
            <?php
                if($_COOKIE['showNSFW'] == "On"){
                    echo <<<_END
                    <script>
...
_END;
                    ...
                    echo <<<_END
                    <script>
                        document.getElementById('Off').selected = 'true';
                    </script>
_END;
                }
            ?>
            <br/><br/>
            <label for='enableDisableUpvote' class='labelClass'>Upvote</label><br/>
            <input type='checkbox'  id='enableDisableUpvote' name='enableDisableUpvote'/><label for='enableDisableUpvote' id='accountCheckboxLabel'>Hide my upvotes in profile</label>
            <?php
                if($_COOKIE['hideUpvotes'] == "on"){
                    echo <<<_END
                    <script>
                        document.getElementById('enableDisableUpvote').checked = true;
                    </script>
_END;
                }
...
_END;
            ...
            <!--<a href='javascript:void(0);' class='blueButton'>Save Changes</a><br/><br/><br/><br/>-->
            <a href='deleteAccount.php' class='blueLink'>Delete my account</a><br/><br/>
       </form>
            <br/><br/><br/>
        </div>
